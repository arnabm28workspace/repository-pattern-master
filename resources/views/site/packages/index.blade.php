                                ...
                                <div class="row">
                                           ...
                                           <div class="s-select">
                                            <select class="sel-basic-pack" id="sel-pack-{{$package->id}}">
                                             @foreach($package->package_duration_price as $price)
                                              <option value="{{$price->price}}" duration="{{$price->duration}}" package-id="{{$package->id}}">{{$price->duration}} Days</option>
                                              @endforeach
                                            </select>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-8">
                                        <div class="form-bx select-feature text-center">
                                          <input type="checkbox" id="chk-lbl-{{$package->id}}" class="package-chk" value="{{$package->id}}" @if($pos==0) {{"checked"}} @endif pack-name="{{$package->name}}" pack-img="{{ asset('storage/'.$package->image) }}" pack-desc="{!!$package->description!!}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-4">
                                     <div class="form-bx-adone text-left float-right mr-3 mt-2">
                                      ...
                                      <label for="bta-{{$package->id}}"><i>Select</i></label>
                                     </div>
                                </div>
                            </div>
...
@endsection
@push('scripts')
<script type="text/javascript" src="{{ asset('frontend/js/package.js') }}"></script>
