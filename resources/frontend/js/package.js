var packageId = $('#package_initial_id').text();
var packageName = '';
var packageImage = '';
var packageDesc = '';
var package_amount = '';
...
$(document).ready(function(){
	packageId = $('.package-chk').val();
	packageName = $('.package-chk').attr('pack-name');
	packageImage = $('.package-chk').attr('pack-img');
  	packageDesc = $('.package-chk').attr('pack-desc');
	var label = 'chk-lbl-'+packageId;
	...
	$(document).on('click', '.add-on-chk', function(){
		ischecked= $(this).is(':checked');
		if(ischecked){
	...
	$('#payment_btn').on('click', function(){
		if(parseInt(package_amount)>0){
			$("#exampleModalCenter").modal();	
		}else{
			$("#payment_btn"). attr('disabled', true);
			//alert("You have to choose at least one basic package to continue");
		}
